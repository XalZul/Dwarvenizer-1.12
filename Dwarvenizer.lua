...
Dwarvenizer.dwarvenize = function(messageIn)
    ...
    for segment in string.gfind(messageIn, "[%p%a%d_ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿĀāĂăĄąĆćĈĉĊċČčĎďĐđĒēĔĕĖėĘęĚěĜĝĞğĠġĢģĤĥĦħĨĩĪīĬĭĮįİıĲĳĴĵĶķĸĹĺĻļĽľĿŀŁłŃńŅņŇňŉŊŋŌōŎŏŐőŒœŔŕŖŗŘřŚśŜŝŞşŠšŢţŤťŦŧŨũŪūŬŭŮ]+") do
        local punctuation = ""

        for p in string.gfind(segment, "(%p+)$") do punctuation = p; break; end
...
Dwarvenizer.print = function(message)
    ...
    for line in string.gfind(message, "(.-)\n") do
